...
int num_threads = 10, num_negative = 5, dim = 18;
int num_vertices;
int *neg_table;
long long total_samples = 1, current_sample_count = 0;
float32 init_rho = 0.025, rho;
...
string *LoadPath(string filename){
    ...
    char str[100];
    string *meta_paths = (string *)calloc(max_num_paths, sizeof(string));
    while(fscanf(fin, "%s", str) != EOF){
        char *strtmp = (char *)calloc(strlen(str)+1, sizeof(char));
        strcpy(strtmp, str);
        if(++num_paths >= max_num_paths){
            max_num_paths += 3;
            meta_paths = (string *)realloc(meta_paths , max_num_paths * sizeof(string));
        }
        meta_paths[num_paths-1] = strtmp;
    }
    fclose(fin);
    meta_paths[num_paths] = NULL;
    return meta_paths;
}
...
bool InitHinproj(string mpath){
    ...
    string *meta_paths = LoadPath(filename);
    for(int k=0; k < num_pairs; k++) checked[k] = false;
    for(int k=0; k < num_pairs; k++){
        for(int j=0; *(meta_paths+j) != NULL; j++)
            if(strcmp(mpairs[k], *(meta_paths+j)) == 0){num_checked--; checked[k]=true; break;}
    }
...
void ReadData(){
    ...
    for(int k=0; k < hinproj->num_bigraph; k++){
        ...
        // Target type (type for learning embeddings)
        if(k == 0){
            int c = 0;
...
struct AliasMat *AliasMethod(SparseMatrix *smat){
    ...
    for(int i=1,s=0,t=0,d=0; i <= rn; i++){
        ...
        for(int j=s; j < t; j++){
            ...
            else if(nprob_ < 1) underfull[num_underfull++] = j;
            else overfull[num_overfull++] = j;
        }
...
void Update(int vs_idx, int target_idx, float32 *vec_error, int label){
    float32 x = 0, g;
    for(int i = 0; i < dim; i++) x += emb_vertex[vs_idx+i] * emb_context[target_idx+i];
    g = (label - FastSigmoid(x)) * rho;
    for(int i = 0; i < dim; i++) vec_error[i] += g * emb_context[target_idx+i];
    for(int i = 0; i < dim; i++) emb_context[target_idx+i] += g * emb_vertex[vs_idx+i];
}
...
void *TrainThread(void *id){
    ...
    long long vs, vt, vs_idx, target, target_idx, label;
    ...
    while(1){
        ...
        for(int i = 0; i < num_negative; i++){
            if(i == 0){
                ...
                label = 0;
                if(target == vt || target == vs) continue;
            }
...
int main(int argc, char **argv) {
    ...
    if (argc == 1) {
		...
		printf("\t-size <int>\n");
		printf("\t\tSet dimension of vertex embeddings; default is 18\n");
		printf("\t-negative <int>\n");
		...
		printf("\nExamples:\n");
		printf("./hpp -train data_directory -output emb.txt -binary 1 -size 128 -negative 5 -samples 10 -rho 0.025 -threads 10\n\n");
		return 0;
	}
